...
export const chunkByDaysSinceToday = (data: Job[]): Job[][] => {
    const groupedByDays = data.reduce((acc, job) => {
        const scrapedDaysAgo = getDaysAgo(job.scrapedDateTimestamp as number);
...
export const getFilters = (jobs: any)  => {
    ...
    const scrapedDaysAgo = new Set(
        jobs.map((job: any) => getDaysAgo(job.scrapedDateTimestamp))
    );
...
console.log(getDaysAgo(timestamp)); // "5 days ago"
